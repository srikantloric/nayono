import { Activity, useEffect, useState, ReactNode } from 'react';
  ...
  return (
    <Stack sx={{ gap: GRID_COMMON_SPACING }}>
        ...
        <Grid container spacing={GRID_COMMON_SPACING}>
          <Grid size={12}>
            <Tabs
              ...
              variant="scrollable"
              slotProps={{ list: { sx: { borderBottom: 'none' } }, indicator: { sx: { display: 'none' } } }}
              sx={{
      ...
      <TabPanel value={value} index={0}>
        <Activity mode={isCart ? 'visible' : 'hidden'}>
          <Cart checkout={cart} onNext={onNext} removeProduct={removeProduct} updateQuantity={updateQuantity} />
        </Activity>
        <Activity mode={!isCart ? 'visible' : 'hidden'}>
          <CartEmpty />
        </Activity>
